...
import tipl.ij.ImageJTypedPath;
...
/**
 * Mader: I have modified this class since the ImageJ version does not return components and has all
 * fields as private making subclassing a huge pain in the ass
 * <p/>
 * <p/>
 * This class is a customizable modal dialog box. Here is an example TIPLDialog with one string
 ...
 * </pre>
 * <p/>
 * To work with macros, the first word of each component label must be unique. If this is not the
 * case, add underscores, which will be converted to spaces when the dialog is displayed. For
 * example, change the checkbox labels "Show Quality" and "Show Residue" to "Show_Quality" and
 * "Show_Residue".
 */
public class IJDialog extends Dialog implements ActionListener, TextListener,
        FocusListener, ItemListener, KeyListener, AdjustmentListener,
        WindowListener, ITIPLDialog {
    /**
     * begin old code *
     */
    ...
    /**
     * Creates a new TIPLDialog with the specified title. Uses the current image image window as the
     * parent frame or the ImageJ frame if no image windows are open. Dialog parameters are recorded
     * by ImageJ's command recorder but this requires that the first word of each label be unique.
     */
    public IJDialog(final String title) {
        this(title,
     ...
     */
    public IJDialog(final String title, final Frame parent) {
    ...
    public static ITIPLDialog.GUIControl asGUI(final Checkbox f) {
        return new ITIPLDialog.CallbackGUIControlWithMouse() {
            @Override
    ...
    public static ITIPLDialog.GUIControl asGUI(final Choice ch) {
        return new ITIPLDialog.CallbackGUIControlWithMouse() {
            @Override
            public String getValueAsString() {
    ...
    public static ITIPLDialog.GUIControl asGUI(final TextField f) {
        return new ITIPLDialog.CallbackGUIControlWithMouse() {
            @Override
            public String getValueAsString() {
                return f.getText();
     ...
     * @param labels        the labels
     * @param defaultValues the initial states
     * @param headings      the column headings Example: http://imagej.nih.gov/ij/plugins/multi
     ...
     * after the user has typed 'OK' or if the dialog has been invoked by a macro; it should read
     * all input fields of the dialog. For other listeners, the OK button will not cause a call to
    ...
    /**
     * Adds a "Help" button that opens the specified URL in the default browser. With v1.46b or
     * later, displays an HTML formatted message if 'url' starts with "<html>". There is an example
     * at http://imagej.nih.gov/ij/macros/js/DialogWithHelp.js
     */
    ...
    public ITIPLDialog.GUIControl appendSlider(final String label, final String helpText,
        ...
        if ((maxValue - minValue) <= 5.0
                && (minValue != (int) minValue || maxValue != (int) maxValue || defaultValue !=
                (int) defaultValue)) {
            scale = 20.0;
            minValue *= scale;
            maxValue *= scale;
    ...
    /**
     * This version allows subpanels to be used seamlessly without modifying every single command
     * here
     */
    public Component[] addComponents(Component[] curObjList) {
        ...
        if ((currentLayer == null) || (currentLayer == this)) {
            ...
            for (int i = 0; i < curObjList.length; i++)
                outResults[i] = currentLayer.add(curObjList[i]);
        }
        return outResults;
    }
    ...
    public GUIControl appendPathField(String label, final String helpText, TypedPath defaultPath) {
        ...
        final Choice thisChoice = new Choice();
        final TextField tf = new TextField(defaultPath.pathString(), columns);
        final String selectFileString = "Select file from disk...";
        final String selectFolderString = "Select folder from disk...";
        thisChoice.addKeyListener(this);
        thisChoice.addItemListener(new ItemListener() {
            @Override
            public void itemStateChanged(ItemEvent e) {
                ...
                if (newValue.equalsIgnoreCase("Other...")) {
                    ...
                    if (newValue.equalsIgnoreCase(selectFileString)) {
                        tf.setText(IJ.getFilePath(selectFileString));
                    } else if (newValue.equalsIgnoreCase(selectFolderString)) {
                        tf.setText(IJ.getDirectory(selectFolderString));
                    } else {
                        tf.setText(newValue);
                    }
        ...
        thisChoice.addItem(selectFileString);
        thisChoice.addItem(selectFolderString);


        for (TypedPath cPath : ImageJTypedPath.listWindows())
            thisChoice.addItem(cPath.pathString());
    ...
    /**
     * Returns an error message if getNextNumber was unable to convert a string into a number,
    ...
    /**
     * Returns a reference to the Label or MultiLineLabel created by the last addMessage() call, or
     * null if addMessage() was not called.
     */
    ...
    /**
     * Returns the contents of the next numeric field, or NaN if the field does not contain a
     * number.
     */
    ...
    /**
     * Returns true if one or more of the numeric fields contained an invalid number. Must be called
     * after one or more calls to getNextNumber().
     */
    ...
    /**
     * Notify any DialogListeners of changes having occurred If a listener returns false, do not
     * call further listeners and disable the OK button and preview Checkbox (if it exists). For
     * PlugInFilters, this ensures that the PlugInFilterRunner, which listens as the last one, is
     * not called if the PlugInFilter has detected invalid parameters. Thus, unnecessary calling the
    ...
    /**
     * Set the insets (margins), in pixels, that will be used for the next component added to the
     * dialog.
     * <p/>
     * <pre>
     * 	    Default insets:
